...
import { BestellungViewDialog } from '@/components/dialogs/BestellungViewDialog';
...
const BESTELLRUNDE_FIELDS = [
  { key: 'runde_name', label: 'Name der Bestellrunde', type: 'string/text' },
  { key: 'restaurant_name', label: 'Restaurant / Lieferdienst', type: 'string/text' },
  { key: 'restaurant_url', label: 'Website / Bestelllink', type: 'string/url' },
  { key: 'bestelldatum', label: 'Bestelldatum und -uhrzeit', type: 'date/datetimeminute' },
  { key: 'deadline', label: 'Deadline für Bestellungen', type: 'date/datetimeminute' },
  { key: 'notiz', label: 'Notiz / Hinweis', type: 'string/textarea' },
];
const BESTELLUNG_FIELDS = [
  { key: 'bestellrunde_auswahl', label: 'Bestellrunde', type: 'applookup/select', targetEntity: 'bestellrunde', targetAppId: 'BESTELLRUNDE', displayField: 'runde_name' },
  { key: 'teilnehmer_vorname', label: 'Vorname', type: 'string/text' },
  { key: 'teilnehmer_nachname', label: 'Nachname', type: 'string/text' },
  { key: 'gerichte_auswahl', label: 'Gewünschte Gerichte', type: 'applookup/select', targetEntity: 'gerichte', targetAppId: 'GERICHTE', displayField: 'gericht_name' },
...
const GERICHTE_FIELDS = [
  ...
  { key: 'gericht_kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'hauptgericht', label: 'Hauptgericht' }, { key: 'dessert', label: 'Dessert' }, { key: 'getraenk', label: 'Getränk' }, { key: 'beilage', label: 'Beilage' }, { key: 'sonstiges', label: 'Sonstiges' }, { key: 'vorspeise', label: 'Vorspeise' }] },
];

const ENTITY_TABS = [
  { key: 'bestellrunde', label: 'Bestellrunde', pascal: 'Bestellrunde' },
  { key: 'bestellung', label: 'Bestellung', pascal: 'Bestellung' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('bestellrunde');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bestellung': return (data as any).bestellung as Bestellung[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'bestellung':
        ...
        lists.gerichteList = (data as any).gerichte ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'bestellung' && fieldKey === 'bestellrunde_auswahl') {
      const match = (lists.bestellrundeList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.runde_name ?? '—';
    }
    if (entity === 'bestellung' && fieldKey === 'gerichte_auswahl') {
      const match = (lists.gerichteList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bestellung': return BESTELLUNG_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'bestellung': return {
        create: (fields: any) => LivingAppsService.createBestellungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBestellungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBestellungEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'bestellung' || dialogState?.entity === 'bestellung') && (
        <BestellungDialog
          open={createEntity === 'bestellung' || dialogState?.entity === 'bestellung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'bestellung' ? handleUpdate : (fields: any) => handleCreate('bestellung', fields)}
          defaultValues={dialogState?.entity === 'bestellung' ? dialogState.record?.fields : undefined}
          bestellrundeList={(data as any).bestellrunde ?? []}
          gerichteList={(data as any).gerichte ?? []}
      ...
      {(createEntity === 'gerichte' || dialogState?.entity === 'gerichte') && (
        <GerichteDialog
          ...
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'gerichte' ? handleUpdate : (fields: any) => handleCreate('gerichte', fields)}
          defaultValues={dialogState?.entity === 'gerichte' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'bestellung' && (
        <BestellungViewDialog
          open={viewState?.entity === 'bestellung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'bestellung', record: r }); }}
          bestellrundeList={(data as any).bestellrunde ?? []}
          gerichteList={(data as any).gerichte ?? []}
        />
      )}
