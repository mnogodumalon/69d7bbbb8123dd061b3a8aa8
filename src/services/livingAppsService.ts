...
import type { Bestellrunde, Bestellung, Gerichte, CreateBestellrunde, CreateBestellung, CreateGerichte } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getBestellung(): Promise<Bestellung[]> {
    ...
    return enrichLookupFields(records, 'bestellung');
  }
  static async getBestellungEntry(id: string): Promise<Bestellung | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Bestellung;
    return enrichLookupFields([record], 'bestellung')[0];
  }
  static async createBestellungEntry(fields: CreateBestellung) {
    return callApi('POST', `/apps/${APP_IDS.BESTELLUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'bestellung') });
  }
  static async updateBestellungEntry(id: string, fields: Partial<CreateBestellung>) {
    return callApi('PATCH', `/apps/${APP_IDS.BESTELLUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'bestellung') });
  }
  ...
  // --- GERICHTE ---
  static async getGerichte(): Promise<Gerichte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERICHTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Gerichte[];
    return enrichLookupFields(records, 'gerichte');
  }
  static async getGerichteEntry(id: string): Promise<Gerichte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GERICHTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Gerichte;
    return enrichLookupFields([record], 'gerichte')[0];
  }
  static async createGerichteEntry(fields: CreateGerichte) {
    return callApi('POST', `/apps/${APP_IDS.GERICHTE}/records`, { fields: cleanFieldsForApi(fields as any, 'gerichte') });
  }
  static async updateGerichteEntry(id: string, fields: Partial<CreateGerichte>) {
    return callApi('PATCH', `/apps/${APP_IDS.GERICHTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gerichte') });
  }
  static async deleteGerichteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GERICHTE}/records/${id}`);
  }
